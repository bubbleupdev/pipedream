...
export default {
	...
	async run({$}) {
		...
		for(let t = 0; t < tasksToBeUpdated.length; t++) {
			let taskData = {};
			let taskGid = await returnGidIfTaskCreated($, this, tasksToBeUpdated[t]);
			if(taskGid === undefined) {
				taskData.data = `${tasksToBeUpdated[t]} task not found after waiting 10+ seconds. ${tasksToBeUpdated[t]} due date not updated.`;
			} else {
				taskData = await updateTaskDueDate($, this, taskGid, dueDateArray[t]);
			}
